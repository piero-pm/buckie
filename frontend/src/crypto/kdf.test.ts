...
// Argon2id derivations are CPU-heavy; whole-suite runs on a loaded dev
// machine exceed the 5s default (same rationale as passphrase tests,
// commit 36728ef).
...
describe('Argon2id KDF (ADR-003)', () => {
  ...
  it(
    'derives the same key for the same passphrase + salt',
    async () => {
