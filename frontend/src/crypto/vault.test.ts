...
// setupVault/unlockVault run Argon2id; whole-suite runs on a loaded dev
// machine exceed the 5s default (same rationale as commit 36728ef).
...
describe('vault verifier (ADR-002/003, EX-PASS-4)', () => {
  ...
  it(
    'refuses a wrong passphrase without revealing data',
    async () => {
  ...
  it(
    'accepts the correct passphrase and unlocks',
    async () => {
...
describe('verifierMatches (BR-IMP-2, wrong-key bundle refusal)', () => {
  it(
    'is true for the key that produced the verifier',
    async () => {
  ...
  it(
    'is false for a key from a different passphrase',
    async () => {
      const { envelope } = await setupVault(4, GOOD)
      const other = await deriveKey('a-different-passphrase', envelope.salt)
      await expect(verifierMatches(other, envelope.verifier)).resolves.toBe(
        false
      )
    },
    KDF_TIMEOUT
